...
export class PresupuestosComponent implements OnInit {
  ...
  public loadAll() {
    const filtrosGuardados = localStorage.getItem('presupuestosFiltros');
  ...
  public filtrarPresupuestos() {
    localStorage.setItem('presupuestosFiltros', JSON.stringify(this.filtros));
    this.loading = true;
    this.apiService.getAll('presupuestos', this.filtros).subscribe(
      (presupuestos) => {
  ...
  public setAnulacion(presupuesto: any, estado: any) {
    ...
    if (confirm('Confirma realización la acción?')) {
      this.apiService.store('presupuesto', presupuesto).subscribe(
        (presupuesto) => {
  ...
  public setPagination(event: any): void {
    this.loading = true;
    this.apiService
      .paginate(this.presupuestos.path + '?page=' + event.page, this.filtros)
      .subscribe(
        (presupuestos) => {
          this.presupuestos = presupuestos;
          this.loading = false;
  ...
  public limpiarFiltros() {
    localStorage.removeItem('presupuestosFiltros');
    this.loadAll();
  }
}
