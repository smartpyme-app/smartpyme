...
export class ProyectosComponent implements OnInit {
  ...
  public loadAll() {
    const filtrosGuardados = localStorage.getItem('proyectosFiltros');
  ...
  public filtrarProyectos() {
    localStorage.setItem('proyectosFiltros', JSON.stringify(this.filtros));
    this.loading = true;
    ...
    this.apiService.getAll('proyectos', this.filtros).subscribe(
      (proyectos) => {
  ...
  public setPagination(event: any): void {
    this.loading = true;
    this.apiService
      .paginate(this.proyectos.path + '?page=' + event.page, this.filtros)
      .subscribe(
        (proyectos) => {
          this.proyectos = proyectos;
          this.loading = false;
  ...
  public delete(proyecto: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.apiService.delete('proyecto/', proyecto.id).subscribe(
          (data) => {
  ...
  public onSubmit() {
    this.saving = true;
    this.apiService.store('proyecto', this.proyecto).subscribe(
      (proyecto) => {
        if (!this.proyecto.id) {
          this.loadAll();
  ...
  public limpiarFiltros() {
    localStorage.removeItem('proyectosFiltros');
    this.loadAll();
  }
}
