...
export class ClientesComponent implements OnInit {
  ...
  public loadAll() {
    const filtrosGuardados = localStorage.getItem('clientesFiltros');
  ...
  public filtrarClientes() {
    localStorage.setItem('clientesFiltros', JSON.stringify(this.filtros));
    this.loading = true;
    this.apiService.getAll('clientes', this.filtros).subscribe(
      (clientes) => {
  ...
  public delete(cliente: any) {
    if (confirm('¿Desea eliminar el Registro?')) {
      this.apiService.delete('cliente/', cliente.id).subscribe(
        (data) => {
  ...
  public setPagination(event: any): void {
    this.loading = true;
    this.apiService
      .paginate(this.clientes.path + '?page=' + event.page)
      .subscribe(
        (clientes) => {
          this.clientes = clientes;
          this.loading = false;
  ...
  public descargarPersonas() {
    ...
    this.apiService
      .export('clientes-personas/exportar', this.filtros)
      .subscribe(
        (data: Blob) => {
  ...
  public descargarEmpresas() {
    ...
    this.alertService.modal = false;
    this.apiService
      .export('clientes-empresas/exportar', this.filtros)
      .subscribe(
        (data: Blob) => {
          const blob = new Blob([data], {
            type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
  ...
  public descargarExtranjeros() {
    ...
    this.alertService.modal = false;
    this.apiService
      .export('clientes-extranjeros/exportar', this.filtros)
      .subscribe(
        (data: Blob) => {
          const blob = new Blob([data], {
            type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
  ...
  public limpiarFiltros() {
    localStorage.removeItem('clientesFiltros');
    this.loadAll();
  }
}
