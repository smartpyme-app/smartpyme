...
export class ProveedoresComponent implements OnInit {
  ...
  public filtrarProveedores() {
    localStorage.setItem('proveedoresFiltros', JSON.stringify(this.filtros));
    this.loading = true;
    this.apiService.getAll('proveedores', this.filtros).subscribe(
      (proveedores) => {
  ...
  public onSubmit() {
    this.saving = true;
    this.apiService.store('proveedor', this.proveedor).subscribe(
      (proveedor) => {
        this.proveedor = {};
        this.saving = false;
  ...
  public setPagination(event: any): void {
    this.loading = true;
    this.apiService
      .paginate(this.proveedores.path + '?page=' + event.page)
      .subscribe(
        (proveedores) => {
          this.proveedores = proveedores;
          this.loading = false;
  ...
  public descargarPersonas() {
    ...
    this.apiService
      .export('proveedores-personas/exportar', this.filtros)
      .subscribe(
        (data: Blob) => {
  ...
  public descargarEmpresas() {
    ...
    this.alertService.modal = false;
    this.apiService
      .export('proveedores-empresas/exportar', this.filtros)
      .subscribe(
        (data: Blob) => {
          const blob = new Blob([data], {
            type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
  ...
  public limpiarFiltros() {
    localStorage.removeItem('proveedoresFiltros');
    this.loadAll();
  }
}
