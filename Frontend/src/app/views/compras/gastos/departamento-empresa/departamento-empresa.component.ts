...
export class DepartamentoEmpresaComponent implements OnInit {
    ...
    public filtrarDepartamento() {
        ...
        this.apiService.getAll('departamentosPlanilla', filtrosParaEnviar).subscribe(departamentos => { 
    ...
    public toggleEstado(departamento: any) {
        ...
        if (confirm(`¿Está seguro de ${nuevoEstado == '1' ? 'activar' : 'desactivar'} este departamento?`)) {
            ...
            this.apiService.store('departamentosPlanilla/changeState/' + departamento.id, departamento).subscribe(response => { 
                this.saving = false;
    ...
    public verAreas(departamento: any) {
        ...
        this.router.navigate(['/gastos/areas-empresa'], { queryParams: { id_departamento: departamento.id } });
    ...
    public onSubmit() {
        ...
        const urlRoute = this.departamento.id ? 'departamentosPlanilla/update' : 'departamentosPlanilla';
    ...
    public delete(id: number) {
        if (confirm('¿Está seguro de eliminar este departamento? Esta acción eliminará también todas las áreas asociadas y no se puede deshacer.')) {
            this.apiService.delete('departamentosPlanilla/', id).subscribe(data => {
    ...
    public descargar() {
        ...
        this.apiService.export('departamentosPlanilla/exportar', this.filtros).subscribe((data: Blob) => {
    ...
    private loadAreas(departamentoId: number) {
        this.apiService.getAll(`departamentosPlanilla/${departamentoId}/areas`).subscribe(areas => { 
            this.areas = areas;
        }, error => {
            this.alertService.error(error);
        });
