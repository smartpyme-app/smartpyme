...
export class GastosComponent implements OnInit {
    ...
    this.apiService.store('gasto', this.gasto).subscribe(
      (gasto) => {
        ...
        this.alertService.success(
          'Gasto guardado',
          'El gasto fue cambiado a ' +
            this.gasto.estado.toLowerCase() +
            ' exitosamente.'
        );
      },
      (error) => {
  ...
  public setRecurrencia(gasto: any) {
    ...
    this.apiService.store('gasto', this.gasto).subscribe(
      (gasto) => {
        ...
        this.alertService.success(
          'Gasto guardado',
          'El gasto se marco como recurrente exitosamente.'
        );
      },
      (error) => {
        this.alertService.error(error);
  ...
  public delete(id: number) {
    if (confirm('¿Desea eliminar el Registro?')) {
      this.apiService.delete('gasto/', id).subscribe(
        (data) => {
  ...
  public setPagination(event: any): void {
    this.loading = true;
    this.apiService
      .paginate(this.gastos.path + '?page=' + event.page, this.filtros)
      .subscribe(
        (gastos) => {
          this.gastos = gastos;
          this.loading = false;
  ...
  public descargar() {
    ...
    this.apiService.export('gastos/exportar', this.filtros).subscribe(
      (data: Blob) => {
  ...
  emitirDTE() {
    this.saving = true;
    this.mhService
      .emitirDTESujetoExcluidoGasto(this.gasto)
      .then((gasto) => {
        this.gasto = gasto;
        this.alertService.success(
          'DTE emitido.',
  ...
  enviarDTE() {
    ...
    this.apiService.store('enviarDTE', this.gasto).subscribe(
      (dte) => {
  ...
  anularDTE(gasto: any) {
    ...
    if (gasto.dte) {
      if (confirm('¿Confirma anular la gasto y el DTE?')) {
        ...
        this.saving = true;
        this.apiService
          .store('generarDTEAnuladoSujetoExcluidoGasto', this.gasto)
          .subscribe(
            (dte) => {
              ...
              this.mhService.firmarDTE(dte).subscribe(
                (dteFirmado) => {
                  this.gasto.dte_invalidacion.firmaElectronica =
                    dteFirmado.body;
                  // this.alertService.success('DTE firmado.');

                  this.mhService
                    .anularDTE(this.gasto, dteFirmado.body)
                    .subscribe(
                      (dte) => {
