...
export class ComprasComponent implements OnInit {
  ...
  public filtrarCompras() {
    localStorage.setItem('comprasFiltros', JSON.stringify(this.filtros));
    this.loading = true;
  ...
  public delete(id: number) {
    if (confirm('¿Desea eliminar el Registro?')) {
      this.apiService.delete('compra/', id).subscribe(
        (data) => {
  ...
  public filtrar(filtro: any, txt: any) {
    this.loading = true;
    this.apiService.read('compras/filtrar/' + filtro + '/', txt).subscribe(
      (compras) => {
        this.compras = compras;
        this.loading = false;
      },
    ...
    this.apiService.store('compra', this.compra).subscribe(
      (compra) => {
        ...
        this.alertService.success(
          'Compra guardada',
          'La compra se marco como recurrente exitosamente.'
        );
      },
      (error) => {
        this.alertService.error(error);
  ...
  public descargarCompras() {
    this.downloadingCompras = true;
    this.saving = true;
    this.apiService.export('compras/exportar', this.filtros).subscribe(
      (data: Blob) => {
  ...
  public descargarDetalles() {
    ...
    this.saving = true;
    this.apiService.export('compras-detalles/exportar', this.filtros).subscribe(
      (data: Blob) => {
        const blob = new Blob([data], {
          type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
        });
  ...
  emitirDTE() {
    this.saving = true;
    this.mhService
      .emitirDTESujetoExcluidoCompra(this.compra)
      .then((compra) => {
        this.compra = compra;
        this.alertService.success(
          'DTE emitido.',
  ...
  enviarDTE() {
    ...
    this.apiService.store('enviarDTE', this.compra).subscribe(
      (dte) => {
  ...
  anularDTE(compra: any) {
    ...
    if (compra.dte) {
      if (confirm('¿Confirma anular la compra y el DTE?')) {
        ...
        this.saving = true;
        this.apiService
          .store('generarDTEAnuladoSujetoExcluidoCompra', this.compra)
          .subscribe(
            (dte) => {
              // this.alertService.success('DTE generado.');
              this.compra.dte_invalidacion = dte;
              this.mhService.firmarDTE(dte).subscribe(
                (dteFirmado) => {
                  this.compra.dte_invalidacion.firmaElectronica =
                    dteFirmado.body;
                  // this.alertService.success('DTE firmado.');

                  this.mhService
                    .anularDTE(this.compra, dteFirmado.body)
                    .subscribe(
                      (dte) => {
                        if (dte.estado == 'PROCESADO' && dte.selloRecibido) {
                          this.compra.dte_invalidacion.sello =
                            dte.selloRecibido;
                        ...
                        this.alertService.success(
                          'DTE anulado.',
                          'El DTE fue anulado exitosamente.'
                        );
                      },
                      (error) => {
