...
export class FlotasComponent implements OnInit {
    ...
    public search(){
        if(this.buscador && this.buscador.length > 2) {
            this.loading = true;
            this.apiService.read('flotas/buscar/', this.buscador).subscribe(flotas => { 
                this.flotas = flotas;
                this.loading = false;
            }, error => {this.alertService.error(error); this.loading = false;});
        }else{
    ...
    public delete(cliente:any){
        if (confirm('¿Desea eliminar el Registro?')) {
            this.apiService.delete('cliente/', cliente.id) .subscribe(data => {
                for (let i = 0; i < this.flotas.data.length; i++) { 
                    if (this.flotas.data[i].id == data.id )
                        this.flotas.data.splice(i, 1);
                }
    ...
    public setPagination(event:any):void{
        this.loading = true;
        this.apiService.paginate(this.flotas.path + '?page='+ event.page).subscribe(flotas => { 
            this.flotas = flotas;
            this.loading = false;
        }, error => {this.alertService.error(error); this.loading = false;});
    }
    ...
    onFiltrar(){
        this.loading = true;
        this.apiService.store('flotas/filtrar', this.filtro).subscribe(flotas => { 
            this.flotas = flotas;
