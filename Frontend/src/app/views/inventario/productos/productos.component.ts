...
export class ProductosComponent implements OnInit {
  ...
  public filtrarProductos() {
    localStorage.setItem('productosFiltros', JSON.stringify(this.filtros));
    this.loading = true;
  ...
  public delete(id: number) {
    if (confirm('¿Desea eliminar el Registro?')) {
      this.apiService.delete('producto/', id).subscribe(
        (data) => {
  ...
  public setPagination(event: any): void {
    this.loading = true;
    this.apiService
      .paginate(this.productos.path + '?page=' + event.page, this.filtros)
      .subscribe(
        (productos) => {
          this.productos = productos;
          this.loading = false;
  ...
  public openModalAjuste(template: TemplateRef<any>, producto: any) {
    ...
    this.inventario = this.producto.inventarios.find(
      (item: any) => item.id_bodega == this.filtros.id_bodega
    );
  ...
  public calAjuste() {
    this.ajuste.ajuste =
      parseFloat(this.ajuste.stock_real) - parseFloat(this.ajuste.stock_actual);
  }
  ...
  public onSubmitAjuste() {
    ...
    this.apiService.store('ajuste', this.ajuste).subscribe(
      (ajuste) => {
  ...
  public limpiarFiltros() {
    localStorage.removeItem('productosFiltros');
    this.loadAll();
  }
}
