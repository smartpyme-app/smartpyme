...
export class ServiciosComponent implements OnInit {
  ...
  public loadAll() {
    const filtrosGuardados = localStorage.getItem('serviciosFiltros');
  ...
  public filtrarServicios() {
    localStorage.setItem('serviciosFiltros', JSON.stringify(this.filtros));
    this.loading = true;
    if (!this.filtros.id_categoria) {
      this.filtros.id_categoria = '';
    }
    this.apiService.getAll('servicios', this.filtros).subscribe(
      (servicios) => {
  ...
  public delete(id: number) {
    if (confirm('¿Desea eliminar el Registro?')) {
      this.apiService.delete('servicio/', id).subscribe(
        (data) => {
  ...
  public setPagination(event: any): void {
    this.loading = true;
    this.apiService
      .paginate(this.servicios.path + '?page=' + event.page, this.filtros)
      .subscribe(
        (servicios) => {
          this.servicios = servicios;
          this.loading = false;
  ...
  public onSubmit() {
    this.loading = true;
    // Guardamos la caja
    this.apiService.store('servicio', this.servicio).subscribe(
      (servicio) => {
        this.servicio = {};
        this.alertService.success(
          'Servicio guardado',
  ...
  public descargar() {
    ...
    this.apiService.export('servicios/exportar', this.filtros).subscribe(
      (data: Blob) => {
  ...
  public limpiarFiltros() {
    localStorage.removeItem('serviciosFiltros');
    this.loadAll();
  }
}
