...
export class ProductoComprasComponent extends BasePaginatedModalComponent implements OnInit {
    ...
    public search(){
        if(this.buscador && this.buscador.length > 2) {
            this.loading = true;
            this.apiService.read('compras/buscar/', this.buscador).pipe(this.untilDestroyed()).subscribe(compras => { 
                this.compras = compras;
    ...
    public filtrar(filtro:any, txt:any){
        this.loading = true;
        this.apiService.read('compras/filtrar/' + filtro + '/', txt).pipe(this.untilDestroyed()).subscribe(compras => { 
            this.compras = compras;
            this.loading = false;
    ...
    onFiltrar(){
        this.loading = true;
        this.apiService.store('compras/filtrar', this.filtro).pipe(this.untilDestroyed()).subscribe(compras => { 
            this.compras = compras;
            this.loading = false; this.filtrado = true;
            this.closeModal();
            this.cdr.markForCheck();
