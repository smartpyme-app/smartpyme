...
/**
 * Tipo gravado efectivo por l├¡nea.
 * Sin IVA en cabecera, una l├¡nea gravada se trata como exenta (no como no_sujeta).
 */
...
/**
 * Al activar/desactivar "Con IVA" en cabecera, ajusta tipo_gravado de las l├¡neas.
 * Gravada sin IVA ÔåÆ exenta; al reactivar IVA solo revierte l├¡neas marcadas autom├íticamente.
 */
...
/** Usuario cambi├│ el tipo manualmente: no revertir a gravada al reactivar IVA. */
export function limpiarExentaPorSinIvaSiTipoManual(detalle: any): void {
  detalle.exenta_por_sin_iva = false;
}

/**
 * Calcula gravada/exenta/no_sujeta, IVA y total con IVA por l├¡nea.
...
/** Suma el total con IVA de cada l├¡nea (redondeado a moneda por l├¡nea). */
...
/**
 * Subtotal de encabezado de venta: por l├¡nea round(precio├ùcantidadÔêÆdescuento, 2) y luego suma.
 * Misma regla que BD y DTE MH (evita drift al sumar detalle.total v├¡a SumPipe).
 */
...
/**
 * Resuelve el % de impuesto a aplicar en una l├¡nea de venta.
