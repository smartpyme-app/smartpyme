...
export class N1coPaymentService {
  ...
  checkAuthenticationStatus(data: {
    ...
    return this.http
      .post(`${this.apiUrl}/payment/check-auth-status`, data)
      .pipe(
        catchError((error) => {
          console.error('Error checking authentication status:', error);
          return throwError(
            () => new Error('Error verificando el estado de autenticación')
          );
        })
      );
  }
  ...
  getExistingPaymentMethod(userId: number): Observable<any> {
    return this.http.get(`${this.apiUrl}/payment/methods/${userId}`).pipe(
      catchError((error) => {
  ...
  processChargeWithExistingMethod(paymentData: any): Observable<any> {
    return this.http
      .post(`${this.apiUrl}/payment/process-ready`, paymentData)
      .pipe(
        catchError((error) => {
  ...
  // Método para actualizar el estado de autenticación 3DS
  changeStatusAuthentication3DS(data: {
    authentication_id: string;
    order_id: string;
    status: 'success' | 'failed';
  }): Observable<any> {
    const url = `${this.apiUrl}/payment/change-status-authentication-3ds`;
