...
export class AlertsHaciendaComponent {
  ...
  get resumenErroresCr(): string {
    const n = this.vistaCostaRica?.detalles.length ?? 0;
    if (n === 0) {
      return '';
    }
    if (n === 1) {
      return 'Se encontró 1 problema';
    }
    return `Se encontraron ${n} problemas`;
  }
  ...
  async copiarRaw(): Promise<void> {
    const raw = this.vistaCostaRica?.raw;
    if (!raw) {
      return;
    }
    try {
      await navigator.clipboard.writeText(raw);
      this.copiadoRaw = true;
      setTimeout(() => {
        this.copiadoRaw = false;
      }, 2500);
    } catch {
      /* ponytail: sin fallback de clipboard en navegadores antiguos */
    }
  }
