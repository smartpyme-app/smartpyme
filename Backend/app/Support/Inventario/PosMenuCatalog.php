...
final class PosMenuCatalog
{
    ...
    public static function queryCategoriasRaiz(int $idEmpresa, bool $soloMostrarEnRestaurante = false): Builder
    {
        $q = Categoria::query()
            ->where('id_empresa', $idEmpresa)
            ->where('enable', true)
            ->where('subcategoria', 0);
        if ($soloMostrarEnRestaurante) {
            $q->where('mostrar_en_restaurante', true);
        }

        return $q->withCount([
            'subcategorias' => function (Builder $sub) use ($idEmpresa, $soloMostrarEnRestaurante) {
                $sub->where('id_empresa', $idEmpresa)->where('enable', true);
                if ($soloMostrarEnRestaurante) {
                    $sub->where('mostrar_en_restaurante', true);
                }
            },
        ])->orderBy('nombre');
    }

    public static function querySubcategorias(int $idEmpresa, int $idCategoria, bool $soloMostrarEnRestaurante = false): Builder
    {
        $q = Categoria::query()
            ->where('id_empresa', $idEmpresa)
            ->where('enable', true)
    ...
    public static function queryProductosDeCategoria(int $idEmpresa, int $idCategoria, bool $soloMostrarEnRestaurante = false): Builder
    {
        return self::queryProductos($idEmpresa, $soloMostrarEnRestaurante)->where('id_categoria', $idCategoria);
    }

    public static function queryProductosDeSubcategoria(int $idEmpresa, int $idSubcategoria, bool $soloMostrarEnRestaurante = false): Builder
    {
        return self::queryProductos($idEmpresa, $soloMostrarEnRestaurante)->where('id_subcategoria', $idSubcategoria);
    }

    /** `imagenes` va eager-loaded porque el accessor `img` de Producto la usa. */
    public static function queryProductos(int $idEmpresa, bool $soloMostrarEnRestaurante = false): Builder
    {
        $q = Producto::query()
            ->with('imagenes')
            ->where('id_empresa', $idEmpresa)
