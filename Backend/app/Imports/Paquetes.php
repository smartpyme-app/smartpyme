...
class Paquetes implements ToModel, WithHeadingRow, WithValidation
{
    ...
    public function model(array $row)
    {
        ...
        if (!$paquete) {
            ...
            $paquete->id_asesor     = isset($row['codigo_asesor']) ? User::where('id_empresa', $usuario->id_empresa)->where('enable', true)->where('codigo', $row['codigo_asesor'])->pluck('id')->first() : null;
