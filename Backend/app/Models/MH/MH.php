...
class MH extends Model
{
    ...
    public function generarDTE($venta){
        ...
        $this->venta->numeroControl = 'DTE-'. $this->venta->tipoDte . '-' . $this->sucursal->cod_estable_mh . $this->caja_codigo . '-' .str_pad($this->venta->correlativo, 15, '0', STR_PAD_LEFT);
    ...
    public function generarDTEAnulado($DTE){
        ...
        // Usar fecha de anulación de la venta si existe, sino usar la fecha actual
        $fechaAnulacion = $this->venta->fecha_anulacion 
            ? \Carbon\Carbon::parse($this->venta->fecha_anulacion)->format('Y-m-d')
