        ...
        try {
            ...
            if ($is_editing) {
                if ($evento->tipo == 'Cancelado') {
                    return response()->json(['error' => 'No se puede editar un evento cancelado'], 400);
                }
                if ($evento->estado == "Pagada") {
                    return response()->json(['error' => 'No se puede editar un evento ya facturado'], 400);
                }
            }

            //validacion de conflicto de eventos
            $inicio = Carbon::parse($request->inicio);
            $fin = Carbon::parse($request->fin);
            $eventosEnConflicto = Evento::where('id', '!=', $evento->id)
            ...
            if ($eventosEnConflicto->count() > 0) {
                return Response()->json([
                    'error' => 'Conflicto de citas',
                    "conflicts" => $eventosEnConflicto,
                    "errorType" => "event_conflict"
                ], 400);
            }
