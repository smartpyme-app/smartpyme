...
class VentasController extends Controller
{
    ...
    public function generarTrasladoEmpresa(Request $request){
        $venta = Venta::with(['detalles', 'cliente'])->find($request->id);
        
        DB::beginTransaction();
        
        try {
            // 1. Crear o buscar el proveedor basado en el cliente
            if($venta->id_cliente){
                $proveedor_id = $this->crearProveedorDesdeCliente($venta->cliente);
            }else{
                $proveedor_id = null;
            }

            // 2. Crear la compra
            $compra = $this->crearCompraDesdeVenta($venta, $proveedor_id);
            
            // 3. Crear los detalles de compra
            $this->crearDetallesCompraDesdeVenta($venta, $compra->id);
            
            DB::commit();
            
            return Response()->json([
                'message' => 'Traslado de empresa generado exitosamente',
                'compra' => $compra->load('detalles', 'proveedor'),
                'venta_original' => $venta
            ], 200);
            
        } catch (\Exception $e) {
            DB::rollback();
            return Response()->json(['error' => 'Error al generar traslado: ' . $e->getMessage()], 500);
        }
    }

    private function crearProveedorDesdeCliente($cliente) {
        // Buscar si ya existe un proveedor con el mismo NIT/DUI
        $proveedor = Proveedor::where(function($query) use ($cliente) {
                if ($cliente->nit) {
                    $query->where('nit', $cliente->nit);
                }
                if ($cliente->dui) {
                    $query->orWhere('dui', $cliente->dui);
                }
            })
            ->first();

        if (!$proveedor) {
            // Crear nuevo proveedor basado en el cliente
            ...
            $proveedor->fill([
                ...
                'cod_distrito' => $cliente->cod_distrito,
                'telefono' => $cliente->telefono,
                'correo' => $cliente->correo,
                'nota' => 'Proveedor creado automáticamente desde cliente - Traslado de empresa',
                'enable' => true,
                'id_usuario' => auth()->id(),
    ...
    private function crearCompraDesdeVenta($venta, $idProveedor) {
        $compra = new Compra();
        $compra->fill([
            ...
            'num_identificacion' => $venta->num_identificacion,
            'referencia' => 'Traslado desde venta #' . $venta->id,
            ...
            'total' => $venta->total,
            'observaciones' => 'Compra generada automáticamente desde venta #' . $venta->id . ' - Traslado de empresa',
    ...
    private function crearDetallesCompraDesdeVenta($venta, $idCompra) {
        foreach ($venta->detalles as $detalleVenta) {
            ...
            $detalleCompra->fill([
                ...
                'no_sujeta' => $detalleVenta->no_sujeta,
                'exenta' => $detalleVenta->exenta,
