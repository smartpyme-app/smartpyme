...
class AreasEmpresaController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = AreaEmpresa::with(['departamento.sucursal', 'departamento.empresa'])
            ->whereHas('departamento', function ($q) use ($usuario) {
    ...
    public function store(Request $request)
    {
        ...
        if ($request->id) {
            $area = AreaEmpresa::whereHas('departamento', function ($q) use ($usuario) {
                $q->where('id_empresa', $usuario->id_empresa);
            })->findOrFail($request->id);
            $area->update($data);
        } else {
            $area = AreaEmpresa::create($data);
        }
    ...
    public function list(Request $request)
    {
        ...
        $query = AreaEmpresa::whereHas('departamento', function ($q) use ($usuario) {
            $q->where('id_empresa', $usuario->id_empresa);
        })
            ->where('activo', true)
            ->where('estado', 1);
    ...
    public function list_departamentos(Request $request)
    {
        ...
        $query = DepartamentoEmpresa::where('id_empresa', $usuario->id_empresa)
            ->where('activo', true);
    ...
    public function exportar(Request $request)
    {
        ...
        if ($usuario->tipo !== 'Administrador') {
            $query->whereHas('departamento', function ($q) use ($usuario) {
                $q->where('id_sucursal', $usuario->id_sucursal);
            });
        }
    ...
    public function cambiarEstadoMultiple(Request $request)
    {
        ...
        $updated = AreaEmpresa::whereHas('departamento', function ($q) use ($usuario) {
            $q->where('id_empresa', $usuario->id_empresa);
        })
