...
<body>
            ...
            <tbody>
                ...
                <tr>
                    <td style="width: 50%; vertical-align: top;">
                        ...
                        <p><b>Correo: </b>{{ $DTE['emisor']['recintoFiscal'] }}</p>
                        <p><b>Correo: </b>{{ $DTE['emisor']['regimen'] }}</p>
                        <br>
                    </td>
                    <td style="width: 50%; vertical-align: top;">
                        @if ($DTE['receptor'])
                            ...
                            <p><b>Tipo de Documento:</b>
                                @if($DTE['receptor']['tipoDocumento'] == '13')>DUI @endif
                                @if($DTE['receptor']['tipoDocumento'] == '36')>NIT @endif
                                @if($DTE['receptor']['tipoDocumento'] == '03')>Pasaporte @endif
                                @if($DTE['receptor']['tipoDocumento'] == '02')>Carnet @endif
                                @if($DTE['receptor']['tipoDocumento'] == '37')>Otro @endif
                            </p>
                            <p><b>Núm de Documento:</b> {{ $DTE['receptor']['numDocumento'] }}</p>
                            <p><b>Act. económica:</b> {{ $DTE['receptor']['descActividad'] }}</p>
                            <p><b>País:</b> {{ $DTE['receptor']['nombrePais'] }} </p>
                            <p><b>Dirección:</b> {{ $DTE['receptor']['complemento'] }} </p>
                            <p><b>Teléfono: </b>{{ $DTE['receptor']['telefono'] }}</p>
                            <p><b>Correo: </b>{{ $DTE['receptor']['correo'] }}</p>
                            <p><b>Nombre Comercial: </b>{{ $DTE['receptor']['nombreComercial'] }}</p>
                        @endif
                    </td>
                </tr>
            </tbody>
        ...
        <table class="table bordered">
            <thead>
                <tr class="bg-light">
                    ...
                    <th width="10%" class="border-bottom text-right">Ventas Afectas</th>
                </tr>
            </thead>
            <tbody>
            ...
            <tfoot>
                <tr>
                    ...
                    <td class="text-right">${{ number_format($DTE['resumen']['totalGravada'], 2) }}</td>
                </tr>
                <tr>
                    <td colspan="4"></td>
                    <td colspan="3">Monto global Desc., Rebajas de operaciones afectas: </td>
                    <td class="text-right">${{ number_format($DTE['resumen']['totalDescu'], 2) }}</td>
                </tr>
                <tr>
                    <td colspan="4"></td>
                    <td colspan="3">Seguro: </td>
                    <td class="text-right">${{ number_format($DTE['resumen']['seguro'], 2) }}</td>
                </tr>
                <tr>
                    <td colspan="4"></td>
                    <td colspan="3">Flete: </td>
                    <td class="text-right">${{ number_format($DTE['resumen']['flete'], 2) }}</td>
                </tr>
                @if(isset($registro->propina) && floatval($registro->propina) > 0)
                    ...
                    <tr>
                        <td colspan="4"></td>
                        <td colspan="3"><b>Total + Propina: </b></td>
                ...
                <tr>
                    <td colspan="4"></td>
                    <td colspan="3" class="bg-light"><b>Total general:</b></td>
        ...
        <table class="table bordered">
            <tbody>
                ...
                <tr>
                    <td colspan="2"><b>Descripción incoterms:</b> {{ $DTE['resumen']['descIncoterms'] }}</td>
                </tr>
            </tbody>
        </table>
