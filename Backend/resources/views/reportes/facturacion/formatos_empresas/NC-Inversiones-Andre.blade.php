...
<body>  
    ...
    <section id="factura">
        <table id="header">
            <tbody style="border: 0px;">
                <tr>
                    ...
                    <td><h1 style="text-align: right; font-size: 1.4em;">NOTA DE CRÉDITO</h1></td>
                </tr>
                <tr>
                    <td>
                        ...
                        <p><b>Dirección: </b> {{ $venta->id_cliente && $cliente ? $cliente->direccion : '' }}</p>
                    </td>
                    <td>
                        <h1 style="color: red; font-size: 1.2em;">000-002-06- {{ str_pad($venta->correlativo, 8, '0', STR_PAD_LEFT)}}</h1>
                        <br>
                        <p><b>FECHA:</b> {{ \Carbon\Carbon::parse($venta->fecha)->format('d/m/Y') }}</p>
                        @if($venta->venta)
                        <p><b>Factura relacionada:</b> 000-002-01- {{ str_pad($venta->venta->correlativo, 8, '0', STR_PAD_LEFT)}}</p>
                        @endif
                        <p><b>ID Cliente:</b> {{ optional($venta->cliente)->codigo_cliente }}</p>
                        <p><b>Cotización:</b> {{ optional($venta->venta)->num_cotizacion }}</p>
                        <p><b>RTN:</b> {{ $venta->id_cliente && $cliente ? $cliente->nit : '' }}</p>
                        <p><b>Teléfono:</b> {{ $venta->id_cliente && $cliente ? $cliente->telefono : '' }}</p>
                    </td>
                </tr>
            </tbody>
        </table>
        ...
        <table id="op">
            ...
            <tbody>
                <tr>
                    <td>{{ optional($venta->venta)->num_orden }}</td>
                    <td>{{ optional($venta->venta)->num_orden_exento }}</td>
                    <td>{{ $venta->id_cliente && $cliente ? $cliente->ncr : '' }}</td>
                    <td></td>
                    <td>{{ optional($venta->venta)->fecha_pago ? \Carbon\Carbon::parse($venta->venta->fecha_pago)->format('d/m/Y') : '' }}</td>
                    <td>{{ optional($venta->venta)->condicion }}</td>
                </tr>
            </tbody>
        </table>
        <br>
        ...
        <table id="productos">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="codigo">   {{ $detalle->codigo }}</td>
            ...
            <tfoot style="display: table-row-group; page-break-inside: avoid;">
                ...
                <tr>
                    <td colspan="3">RANGO 000-002-06-00000201 / 000-002-06-00000220</td>
